...
当前发布版本以 **iOS 15.0** 为最低部署目标，采用原生 SwiftUI 框架保留截图所示的三个底部入口：**首页**、**我的**与**搜索**。底部导航使用半透明材质、胶囊轮廓与粉色选中态呈现玻璃感效果；入口选择会在本机保存并于下次启动恢复。首页请求公开推荐数据并显示原生视频卡片，搜索页请求聚合搜索结果，点击视频后使用系统 `AVPlayer` 播放；播放器可将当前画面保存到系统相册。“我的”页提供二维码登录状态、系统钥匙串会话存储、退出登录和图片保存入口。

手机号验证码登录与账号密码登录会打开官方登录页面。手机号、验证码和密码不会在本应用的界面、源代码、日志或仓库中读取和保存；扫码成功后的会话状态仅按系统钥匙串策略保存在设备本机。
...
| 内容框架 | 公开推荐、聚合搜索、原生视频播放与账号状态页 | 视频、弹幕、动态与直播 |
| 保存能力 | 系统图片保存与当前视频画面保存 | 原图、GIF、Live Photo 保存 |
